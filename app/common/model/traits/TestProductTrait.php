...
/**
 * 你把当前trait文件理解为模型文件【app\common\model\TestProduct】的一部分，自定义代码都定义在当前文件中
 */
...
trait TestProductTrait
{
    ...
    public function test3($list, $rule, $page)
    {
        return '最大:' . max($list);
    }

    public function test2($list, $rule, $page)
    {
        if (count($list) == 0) {
            return ' ';
        }
        return array_sum($list) / count($list) . '|' . max($list);
    }
